...
- **错误日志与告警**：系统功能报错自动记录（限流/超时/网络/数据缺失分类，60s 节流去重；保留期 30 天且上限 2000 条，写入时自动清理超期记录）；**数据源被限流是最关键分类**，记录触发模块与接口（如 `akshare.stock_zh_a_spot_em`、`realtime.eastmoney`、`patch.nid_auth`），用于排查哪个模块触发限制。前端右上角铃铛实时告警（60s 轮询未读徽标，点击看最近 10 条）；系统设置页支持分类筛选、**下载日志文件**与清空
- **提示功能开关**：系统设置页两个独立开关——「场内提示与扫描」「场外提示与高低估」，关闭后对应模块不再生成提示且**不发起其数据源请求**（省资源），立即生效
- **主动基金高低估近似映射**：基准为宽基组合的主动基金，取基准中占比最高的支持指数近似映射（≥70% 视为直接跟踪、20~70% 标注"近似"、<20% 不提示）；固收+/偏债 基金权益占比低自动排除
- **概念板块映射（渐进获取）**：同花顺 375 概念板块成分映射表——起步数据内置（WorkBuddy 抓取的各板块核心成分，零网络导入），渐进任务每轮抓 20 个最久未更新板块（防封禁 sleep），副标签真概念优先、关键词规则回退永不空窗；系统设置页显示映射进度与手动抓取按钮
- **重仓股重叠度与组合 X 光**：基金 PK 选定基金自动计算共同重仓排行（Jaccard 重叠矩阵）；**组合 X 光透视**（借鉴 fundadvisor portfolio_xray 可解释扣分制）——持仓穿透到个股（HHI/等效持股数）、两两基金重叠矩阵、经理/公司单点依赖告警（≥35% 经理/≥60% 公司）、100 分制多样化评分（逐项引用扣分原因），REST `/api/funds/xray` + PK Tab + AI Skill 三通道
- **调仓建议自进化**：调仓工单落库 → 30 天窗口自动回填实际涨跌 → sell/buy 分类命中率 → 止盈/止损阈值保守校准（上下限约束）；`POST /api/analysis/advice-eval` 手动或调度触发
...
```bash
# 1. 配置环境变量（数据库/端口等基础项；AI Key 与飞书 Webhook
#    不在 .env 配置，启动后在 Web 界面「系统设置/推送配置」中填写）
cp .env.example .env
...
> 推荐日常节奏：工作日盘中看仪表盘实时估值 → 每日收盘后定时分析+推送（自动）→ 周末看投资复盘和全量回测。
...
1. 选**起始/结束日期**（默认最近 30 天）→ 点「开始复盘」；
2. 三张汇总卡：组合等权区间收益、沪深300 同期（跑赢/跑输 pp）、**信号命中率**（区间首日 buy/sell 信号与实际涨跌的同向率——量化你的信号质量）；
3. 明细表：逐只涨跌、净值、评分变化、信号迁移（始→末）；
4. 点「AI 解读」将完整报告发给 AI（用你配置的模型）生成风险解读与观察点。
- 口径：等权买入持有（期间无调仓假设）；基准为沪深300 官方指数点位。

### 5. 信号回测与自动全量回测（/backtest）

- **单基金回测**：选基金 + 天数（30~1500）→ 运行。ECharts 双轴图（净值 vs 策略累计收益）+ 逐点有效性评分。信号次日生效（next-bar）、几何复利；非交易日信号自动对齐下一交易日。
- **自动全量回测**：
  1. 打开「自动全量回测」开关（每周日 00:00 自动运行）；
  2. 「间隔下限/上限(秒)」控制每只基金之间的随机等待（默认 20~60s，周末低峰防封禁；60 只基金约 30~60 分钟，12 小时兜底）；
  3. 结果逐基金落库并标注**完成时间**，批量结果表实时刷新；单只失败标红不影响其余；
...
清盘风险/规模冲击/仓位漂移/机构认可度四项读 `fund_quarterly`（按 `effective_date <= 今天` 取已披露报告期），该表由「刷新数据」在写 pingzhongdata 扩展缓存时同源落库，不额外发起请求；未跑过详情刷新时表为空，四项按中性处理。

### 实时净值预估（展示层，不参与信号）

仪表盘"实时估值"列与报告"前十大持仓涨跌"为盘中估算：场外基金优先天天基金官方估值（fundgz），不可用时按最新季报 top10 持仓 × 个股实时涨跌加权自算（东财→腾讯→新浪三级个股源）；持仓只覆盖股票仓位的一部分，未覆盖的 `R_stock - 覆盖率` 份额按当日指数涨跌补齐（`est_model=position_aware`，R_stock 取该基金最新已生效季报股票仓位），缺季报仓位时回退旧口径（覆盖率 ≥50% 归一法、<50% 指数混合）。**分析信号始终基于官方净值计算**，估算值仅供盘中参考。
